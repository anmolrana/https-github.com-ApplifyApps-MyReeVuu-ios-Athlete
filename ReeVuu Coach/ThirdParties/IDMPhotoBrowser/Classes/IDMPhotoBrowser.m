...
- (void)performPresentAnimation {
  ...
  //  _senderViewOriginalFrame = CGRectMake(0, ([UIScreen mainScreen].bounds.size.height - imageFromView.size.height)/2, imageFromView.size.width, imageFromView.size.height);
...
- (void)viewDidLoad {
    ...
    if (_displayBackButton) {
        ...
        _backButton.frame = CGRectMake(0,10, 56, 56);
        [_backButton setImage:[UIImage imageNamed:@"ic_back_w"] forState:UIControlStateNormal];
        //  [_backButton setFrame:[self frameForDoneButtonAtOrientation:currentOrientation]];
        CGRect r = [self frameForDoneButtonAtOrientation:currentOrientation];
        r.origin.x = 0;
        _backButton.frame = r;
        [_backButton setAlpha:1.0f];
        [_backButton addTarget:self action:@selector(doneButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    }
   ...
   // if (_displayPLayButton) {
        ...
        [_playButton setImage:[UIImage imageNamed:@"ic_play_circle"] forState:UIControlStateNormal];
        //  [_backButton setFrame:[self frameForDoneButtonAtOrientation:currentOrientation]];
        CGRect r = CGRectMake(self.view.frame.size.width/2-67/2, self.view.frame.size.height/2-67/2, 67, 67);
        _playButton.frame = r;
        [_playButton setAlpha:1.0f];
   ...
   // }
    ...
    if(!_doneButtonImage) {
        ...
        [_doneButton setTitle:IDMPhotoBrowserLocalizedStrings(@"Exit") forState:UIControlStateNormal];
...
- (void)performLayout {
    ...
    if(_displayBackButton && !self.navigationController.navigationBar)
        [self.view addSubview:_backButton];
    
    //if(_displayPLayButton)
        [self.view addSubview:_playButton];
    if(_displayDeleteButton && !self.navigationController.navigationBar)
...
-(void)showPlayButton :(BOOL)show{
    [_playButton setHidden:!show];
    _displayPLayButton = show;
    _playButton.alpha = 1;
...
- (id<IDMPhoto>)photoAtIndex:(NSUInteger)index {
    if (_photos.count==0|| _photos.count <= index) {
        return nil;
    }
...
- (void)DeletePressed:(id)sender {
    NSString*strText = _displayPLayButton?@"Are you sure you want to delete video?":@"Are you sure you want to delete image?";
    strText = @"Are you sure you want to delete image or video?";
    UIAlertController*Av = [UIAlertController alertControllerWithTitle:nil message:strText preferredStyle:UIAlertControllerStyleAlert];
    [Av addAction:[UIAlertAction actionWithTitle:@"Yes" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [_photos removeObjectAtIndex:_currentPageIndex];
        [self reloadData];
        [_delegate DeletePhotoAtIndex:_currentPageIndex AndBrowser:self];
...
//        }
    ...
    [Av addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
